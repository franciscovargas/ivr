...
ftng = findtfeat(histg,edges,sizeparam,1);
ftnb = findtfeat(histb,edges,sizeparam,1);

[height, width, color] = size(normed);
output = zeros(height, width,3);
for row = 1 : height
    for col = 1 : width
        if normed(row,col,1) < ftnr ... % inside high bnd
        & normed(row,col,1) >  0.0 % optional low bnd
            output(row,col,1) = 1;
        else
            output(row,col,1) = 0;
        end
...
for row = 1 : height
    for col = 1 : width
        if normed(row,col,2) < ftng ... % inside high bnd
        & normed(row,col,2) > 0.0 % optional low bnd
            output(row,col,2) = 1;
        else
            output(row,col,2) = 0;
        end
...
for row = 1 : height
    for col = 1 : width
        if normed(row,col,3) <  ftnb ... % inside high bnd
        & normed(row,col,3) >  0.0 % optional low bnd
            output(row,col,3) = 1;
        else
            output(row,col,3) = 0;
        end
